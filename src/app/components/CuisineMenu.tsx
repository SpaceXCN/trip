...
import { ArrowLeft, Utensils, Info, MapPin, X } from "lucide-react";
...
export function CuisineMenu() {
  ...
  // Find the cuisine from the 8 great cuisines
  const cuisineItem = CUISINES.find(c => c.id === cuisineId);
    ...
    return (
      <div className="w-full min-h-screen bg-background flex flex-col items-center justify-center p-8">
        <Seo
          ...
          description="This Chinese cuisine guide could not be found."
          path={`/cuisine/${cuisineId || ""}`}
          noindex
        />
        <h2 className="text-xl font-medium text-foreground mb-4">Cuisine not found</h2>
        <Link to="/cuisines" className="text-primary hover:underline">
          Return to Cuisines Hub
        </Link>
      </div>
    );
  }

  const cuisinePath = `/cuisine/${cuisineItem.id}`;
  const pageDescription = truncateDescription(
    `Explore ${cuisineItem.name}, its regional background, spice level, and signature dishes including ${cuisineItem.signatureDishes.join(", ")}.`,
  );
  const cuisineFaqs = [
    {
      question: `What is ${cuisineItem.name} known for?`,
      answer: cuisineItem.description,
    },
    {
      question: `Where is ${cuisineItem.name} popular?`,
      answer: `${cuisineItem.name} is associated with ${cuisineItem.region}.`,
    },
    {
      question: `What dishes should I try from ${cuisineItem.name}?`,
      answer: `Start with ${cuisineItem.signatureDishes.join(", ")}.`,
    },
  ];
  const menuJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `${cuisineItem.name} Signature Dishes`,
    description: pageDescription,
    itemListElement: cuisineItem.menuItems.map((item, idx) => ({
      "@type": "ListItem",
      position: idx + 1,
      item: {
        "@type": "MenuItem",
  ...
  return (
    <div className="w-full pb-20 bg-background min-h-screen">
      <Seo
        title={`${cuisineItem.name} Guide and Signature Dishes`}
        description={pageDescription}
        path={cuisinePath}
        image={cuisineItem.image}
        keywords={[cuisineItem.name, "Chinese cuisine", ...cuisineItem.signatureDishes]}
        jsonLd={[
          buildWebPageJsonLd({
            path: cuisinePath,
            name: `${cuisineItem.name} Guide and Signature Dishes`,
            description: pageDescription,
            image: cuisineItem.image,
          }),
          buildBreadcrumbJsonLd([
            ...
            { name: cuisineItem.name, path: cuisinePath },
          ]),
          menuJsonLd,
      ...
      <div className="relative h-[400px] w-full">
        <img 
          ...
          alt={cuisineItem.name} 
          className="w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent"></div>
        <div className="absolute top-8 left-8 z-10">
          <Link to="/cuisines" className="bg-background/20 hover:bg-background/40 backdrop-blur-md text-white p-3 rounded-full flex items-center justify-center transition-colors shadow-sm">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 max-w-4xl mx-auto">
          <div className="text-white">
            <span className="bg-primary text-primary-foreground text-xs px-3 py-1.5 rounded-full uppercase tracking-wider mb-4 inline-block shadow-sm font-medium">
              {cuisineItem.chineseName}
            </span>
            <h1 className="mb-4 text-white drop-shadow-md">{cuisineItem.name}</h1>
            <p className="text-lg opacity-90 max-w-2xl flex items-center gap-2">
              <MapPin className="w-5 h-5"/> Popular in: {cuisineItem.region}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        <div className="mb-10 text-muted-foreground text-lg leading-relaxed">
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 md:p-8 border-b border-border bg-muted/30">
            ...
            <p className="text-muted-foreground mt-2 text-sm flex items-center gap-2">
              <Info className="w-4 h-4"/> Discover the iconic flavors that define this culinary tradition.
            </p>
          </div>
          
          <div className="divide-y divide-border">
            {cuisineItem.menuItems && cuisineItem.menuItems.length > 0 ? (
              cuisineItem.menuItems.map((item, idx) => (
                <div 
                  key={idx} 
              ...
              <div className="p-8 text-center text-muted-foreground">
                Detailed menu items are being prepared for this cuisine. Check back soon!
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          <Link to="/cuisines" className="inline-flex items-center gap-2 bg-primary hover:opacity-90 text-primary-foreground px-8 py-3 rounded-lg transition-colors font-medium">
            Explore more cuisines <ArrowLeft className="w-4 h-4 rotate-180" />
          </Link>
        </div>

        <FAQSection title={`${cuisineItem.name} Questions`} faqs={cuisineFaqs} />
      </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 md:p-10 bg-card">
              <div className="prose prose-lg dark:prose-invert max-w-none">
                ...
                <div className="mt-8 pt-8 border-t border-border flex items-center gap-4 text-muted-foreground italic">
                  ...
                  <p className="m-0 text-sm">Experience the authentic flavors of {cuisineItem.name}.</p>
                </div>
              </div>
            </div>
          </div>
